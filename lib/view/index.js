...
export default class Root extends React.PureComponent {
  ...
  async search() {
    ...
    var url = "https://api.qwant.com/api/search/images?count=100&t=images&safesearch=0&uiv=4&q="+query.join(",")
    ...
    await fetch(url)
      ...
      .then(res => {
        ...
        for (var pic of res.items) {
          pics.push({url : pic.link, preview: pic.image.thumbnailLink})
        }
      }).catch(err => {
    ...
    await fetch(url, {
      ...
      .then(res => {
        ...
        for (var pic of res.photos) {
          pics.push({url : pic.src.large, preview: pic.src.medium})
        }
      })
      .catch(err => {
